...
const DevicesList = ({ title = "Appareils" }) => {
    ...
    useEffect(() => {
        const checkUserAndToken = () => {
            if (whoAuth.isCurrentUserManager() || whoAuth.isCurrentUserAdmin()) {
                navigate('/403');
            }
    ...
    async function getDevices() {
        try {
            let result = await ClientService.findDevicesByEmail(getEmail());
    ...
    const handleExport = () => {
        if (selectedRows.length === 0) {
            ...
            const data = [
                ['id', 'imei', 'statusDevice', 'firmware', 'configuration'],
                ...selectedRows.map((row) => [row.id, row.imei, row.statusDevice, row.firmware, row.configuration,]),
            ];
                    ...
                    <div>
                        <div className="flex justify-end mb-1.5 ">
                            <Button
                                ...
                                text="Exporter"
                                className="btn-dark rounded-[999px] px-2 py-1 text-xs md:px-4 md:py-2 md:text-sm mr-3"
